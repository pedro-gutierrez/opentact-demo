 (function() {
    ...
    Vue.use( function( V, options ) { 
      ...
      V.prototype.$_receive = function( e, args ) {
        if( args && args.simpleCopy ) args = args.simpleCopy();
        var fsm = this;
        var e = e.toLowerCase();
        this.e = e;
        if( this.current ) {
      ...
      Vue.$resolveValue = function( prop, source ) {
        ...
        if( "string" === typeof( spec ) ) {
          ...
          if( spec instanceof Array ) {
            spec.map( function(prop){
              args[prop.split(".").$last()]= Vue.$resolveValue( prop, source )
            });

          } else {
            spec.$map(function(k, v) {
              args[k]= Vue.$resolveValue( v, source )      
            });
          }
  ...
  Vue.$clearValue = function( source, spec ) {
    if( source[spec] ) {
      switch (typeof source[spec] ) {
        case "string": 
          source[spec]=null;
          break;
        case "number":
          ...
          break;
        default:  
          switch( source[spec].constructor ){
            case Array:
              source[spec] = [];
              break;
            default:
